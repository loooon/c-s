...
public class DunYellowPageController extends AbstractBaseController
{
    ...
    @Value("${dun.batch.query.max.times}")
    private Integer dunBatchQueryMaxTimes;
    ...
    public ResultJson index(String phone, HttpServletRequest request)
    {
        ...
        try
        {
            ...
            if (isFluentQuery(singleIpCache,ip,dunQueryMaxTimes))
            {
    ...
    private static final ICache<String, Integer> singleIpCache = CacheFactory.getInstance()
            .newCache(1, TimeUnit.DAYS, 100000);

    private static final ICache<String, Integer> batchIpCache = CacheFactory.getInstance()
            .newCache(1, TimeUnit.DAYS, 100000);

    /*private boolean isFluentQuery(ICache<String, Integer> ipCache, String ip,Integer limitedQueryTimes) throws Exception
    {
    ...
    public ResultJson batch(HttpServletRequest request)
    {
        ...
        {
            String ip = RequestUtil.gettRequestIP(request);
            if (isFluentQuery(batchIpCache,ip,dunBatchQueryMaxTimes))
            {
                return new ResultJson().error(ResultJson.STATUS_CODE_FAIL, "频率过快，请稍后再试!");
            }
        } catch (Exception e)
